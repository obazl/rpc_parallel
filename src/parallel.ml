...
let get_state_exn () =
  ...
  | None ->
      failwith "State should have been set already"
  | Some state ->
      state
...
module Function = struct
  ...
  let maybe_generate_name ~prefix ~name =
    match name with
    | None ->
        sprintf "%s_%s" prefix (Rpc_id.to_string (Rpc_id.create ()))
    | Some n ->
        n
  ...
  module Function_reverse_piped = struct
    ...
    let make_master t ~implement ~ok ~error =
      implement t.master_rpc (fun () id ->
          ...
          | None ->
              [%message
                "Bug in Rpc_parallel: reverse pipe master implementation not \
                 found"
                  (id : Id.t)
                  (Rpc.Pipe_rpc.name t.master_rpc : string)]
              |> Deferred.Or_error.error_s |> error )

    let make_master_impl t =
      make_master t ~implement:Rpc.Pipe_rpc.implement
        ~ok:Deferred.Or_error.return ~error:ident

    let make_master_impl_direct t =
      make_master t ~implement:Rpc.Pipe_rpc.implement_direct ~ok:ident
        ~error:const
  ...
  let reverse_pipe ~make_master_impl ~monitor ~name ~f ~bin_query ~bin_update
      ~bin_response =
    let proto =
  ...
  let create_reverse_direct_pipe ~monitor ~name ~f ~bin_query ~bin_update
      ~bin_response =
    reverse_pipe
      ~make_master_impl:Function_reverse_piped.make_master_impl_direct ~monitor
  ...
  let run_internal (type query response)
    ...
    | Plain proto ->
        Rpc.Rpc.dispatch proto connection arg
...
let init_master_state ~rpc_max_message_size ~rpc_handshake_timeout
  ...
  match Set_once.get global_state with
  | Some _state ->
      failwith "Master state must not be set up twice"
  | None ->
      ...
      let my_server =
        lazy
          (let%bind server =
           ...
           let host =
             if Or_error.ok_exn os = "Darwin\n" then "localhost"
             else Unix.gethostname ()
           in
           Host_and_port.create ~host ~port:(Tcp.Server.listening_on server))
      in
      let as_master =
...
module Make (S : Worker_spec) = struct
  ...
  let run_executable where ~env ~worker_command_args ~input =
    ...
    | Executable_location.Local ->
        Utils.our_binary ()
        >>=? fun our_binary ->
        Process.create ~prog:our_binary ~argv0:Sys.argv.(0)
  ...
  module Function_creator = struct
    ...
    let reverse_pipe ~create_function ?name ~f ~bin_query ~bin_update
        ~bin_response () =
    ...
    let create_reverse_pipe ?name ~f ~bin_query ~bin_update ~bin_response () =
      reverse_pipe ~create_function:Function.create_reverse_pipe ?name ~f
        ~bin_query ~bin_update ~bin_response ()

    let create_reverse_direct_pipe ?name ~f ~bin_query ~bin_update
        ~bin_response () =
      reverse_pipe ~create_function:Function.create_reverse_direct_pipe ?name
  ...
  let decorate_error_if_running_inline_test error =
    let tag =
      "You must call [Rpc_parallel.For_testing.initialize] at the top level \
       before any tests are defined. See \
       lib/rpc_parallel/src/parallel_intf.ml for more information."
    in
  ...
  let spawn_process ~where ~env ~cd ~name ~connection_timeout ~daemonize_args =
    ...
    | None ->
        let error =
          Error.of_string
            "You must initialize this process to run as a master before \
             calling [spawn]. Either use a top-level [start_app] call or use \
             the [Expert] module."
        in
        return (Error (decorate_error_if_running_inline_test error))
    | Some global_state ->
        Deferred.Or_error.return global_state.as_master )
    >>=? fun global_state ->
    ...
    let worker_command_args =
      ...
      | Decorate token ->
          "RPC_PARALLEL_WORKER" :: token :: Option.to_list name
      ...
      | Ok (id, process) -> (
          ...
          | Error _ as error ->
              don't_wait_for
                ...
                >>| (ignore : Unix.Exit_or_signal.t -> unit) ) ;
              return error
          | Ok worker ->
              Deferred.Or_error.return (worker, process) )
    in
  ...
  let wait_for_daemonization_and_collect_stderr name process =
    ...
    | Error _ ->
        let error =
          Error.createf "Worker process %s"
...
let worker_main ~worker_env =
  ...
  let register my_host_and_port =
    ...
    with
    | Error exn ->
        failwiths "Worker failed to register" exn [%sexp_of: Exn.t]
    | Ok (Error e) ->
        failwiths "Worker failed to register" e [%sexp_of: Error.t]
    | Ok (Ok `Shutdown) ->
        failwith "Got [`Shutdown] on register"
    | Ok (Ok `Registered) ->
        ()
  in
...
module Expert = struct
  ...
  let worker_init_before_async_exn () =
    ...
    | `Worker ->
            ...
            raise_s
              [%message
                "Unable to read worker config from stdin. Make sure nothing \
                 is read from stdin before [worker_init_before_async_exn] is \
                 called."
                  (exn : Exn.t)]
        in
        let maybe_release_daemon =
          ...
          | `Don't_daemonize ->
              Core.Unix.chdir config.cd ; Fn.id
          | `Daemonize {Daemonize_args.umask; redirect_stderr; redirect_stdout}
            ->
  ...
  let start_master_server_exn ?rpc_max_message_size ?rpc_handshake_timeout
      ?rpc_heartbeat_config ?(pass_name = true) ~worker_command_args () =
    match Utils.whoami () with
    | `Worker ->
        failwith "Do not call [init_master_exn] in a spawned worker"
    | `Master ->
        init_master_state ~rpc_max_message_size ~rpc_handshake_timeout
          ~rpc_heartbeat_config
          ~worker_command_args:
            (User_supplied {args= worker_command_args; pass_name})
...
module For_testing = struct
  let initialize here =
    if am_running_inline_test then
      match Utils.whoami () with
      | `Master -> (
          ...
          | Some `started ->
              ()
          | None ->
              Expert.start_master_server_exn ~worker_command_args:[] () )
      | `Worker ->
          if For_testing_internal.worker_should_initialize here then (
...
let start_app ?rpc_max_message_size ?rpc_handshake_timeout
  ...
  | `Master ->
      ...
      in
      init_master_state ~rpc_max_message_size ~rpc_handshake_timeout
        ~rpc_heartbeat_config ~worker_command_args:(Decorate decoration) ;
      Command.run command
